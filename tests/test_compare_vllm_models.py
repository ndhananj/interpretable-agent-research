...
class _CompareHandler(BaseHTTPRequestHandler):
    ...
    adapter_returns_bad_edit = False
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        edit_text = "TODO\n" if model == "adapter-model" and self.__class__.adapter_returns_bad_edit else "DONE\n"
        self._write_json(
            {
                "choices": [
                    {
...
def fake_server() -> str:
    ...
    _CompareHandler.adapter_returns_bad_edit = False
...
def test_reports_include_both_models_and_score_details(compare_config: Path, tmp_path: Path) -> None:
    ...
    assert saved_summary["tasks"][0]["diagnostics"]["check_deltas"] == ["no pass deltas"]
...
def test_reports_warn_on_per_check_lora_regression(compare_config: Path, tmp_path: Path) -> None:
    _CompareHandler.adapter_returns_bad_edit = True
    ...
    diagnostics = summary["tasks"][0]["diagnostics"]
    assert any("base=True lora=False" in delta for delta in diagnostics["check_deltas"])
    assert diagnostics["likely_failure_category"] == "action_regression"
    assert summary["warnings"]
    report = (tmp_path / "compare" / "report.md").read_text(encoding="utf-8")
    assert "Regression Warnings" in report
